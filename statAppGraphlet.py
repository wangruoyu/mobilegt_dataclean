...
for name in flowFeatureFileNames:
	...
	appName_graphlets={}
	appName_flowNums={}		#{appName:flowNums} flowNums={created:flowNum}	
	appName_pktNums={}		#{appName:pktNums}	pktNums={created:pktNum}
	appName_byteNums={}		#{appName:byteNums}	byteNums={created:byteNum}
	for line in flowFeatureFile:
		#解析每条流，按流的应用类别分组构建graphlet，每个graphlet包含的流在5分钟内(即第一条流的created时间与最后一条流的created时间差不超过300秒<流不是按创建时间的从小到大的顺序?该问题应如何处理?>)
		...
		if appName not in appName_flowNums:
			appName_flowNums[appName]={}
			appName_pktNums[appName]={}
			appName_byteNums[appName]={}
		if appName_flowNums[appName].has_key(foundcreated):
			flowNum+=appName_flowNums[appName][foundcreated]
			pktNum+=appName_pktNums[appName][foundcreated]
			byteNum+=appName_byteNums[appName][foundcreated]
		appName_flowNums[appName][foundcreated]=flowNum
		appName_pktNums[appName][foundcreated]=pktNum
		appName_byteNums[appName][foundcreated]=byteNum
	...
	for appName in appName_graphlets:
		...
		for createdTime in appName_graphlets[appName]:
			...
			statGraphletResult.write(str(len(appNode0))+delimiter+str(len(appNode1))+delimiter+str(len(appNode2))+delimiter+str(len(appNode3))+delimiter+str(len(appNode4)))
