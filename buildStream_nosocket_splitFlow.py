...
def main(argv):
	...
	for WORKING_DIR in WORKING_DIR_LIST:
		...
		#无socket文件,flowApp自动填充数据
		flowApp={}
		#flowApp[flowkey][index][0]------name--------NO_SOCKET
		#flowApp[flowkey][index][1]------created-----NULL
		#flowApp[flowkey][index][2]------destroyed---NULL	
		...
		for name in flowFiles:
			...
			#无socket文件,flowApp自动填充数据
			if not flowApp.has_key(flowkey_skipIpUser):
				appName='NO_SOCKET'
				created='NULL'
				destroyed='NULL'
				flowApp[flowkey_skipIpUser]=[]
				flowApp[flowkey_skipIpUser].append([appName,created,destroyed])
			...
			for index in range(len(flowApp[flowkey_skipIpUser])):
				...
				for split_index in pktTSs_all[index]:
					...
					if len(pktTSs_all[index][split_index])<MIN_PKTNUM or len(in_pktTSs_all[index][split_index])<MIN_PKTNUM or len(out_pktTSs_all[index][split_index])<MIN_PKTNUM:
						#print 'flow '+flowKey+'( '+flowkey_skipIpUser+') split_index ['+str(split_index)+'] in pkts num:'+str(len(in_pktTSs_all[index][split_index]))+'\tout pkts num:'+str(len(out_pktTSs_all[index][split_index]))
						ff_exceptionLog.write('flow %s( %s) split_index[%s] IN pkts num:%s\tOUT pkts num:%s\n' %(flowKey,flowkey_skipIpUser,split_index,len(in_pktTSs_all[index][split_index]),len(out_pktTSs_all[index][split_index])))
						continue
